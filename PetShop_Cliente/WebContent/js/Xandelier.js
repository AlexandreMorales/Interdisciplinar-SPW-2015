...
function show(div, value, delay) {
    delayShowBool = true;
    valueShowBool = value;
    var element = (getType(div) === "string") ? getElement("#" + div) : div;
    if (value == undefined)
    ...
    if (delay === undefined)
        ...
        delayShowBool = false;
        var op = value ? 0.1 : 1;  // initial opacity
        var timer = setInterval(function () {
            element.style.display = value ? 'block' : 'none';
            if (op > 1 || op < 0.1 || delayShowBool) {
                clearInterval(timer);
                element.style.display = valueShowBool ? 'block' : 'none';
            }
    ...
    switch (item[0]) {
        ...
        case '+':
            ...
            switch (item[1]) {
                ...
                case "<=": auxFilter = el => parseFloat(el[funcGetAtt](item[0])) <= parseFloat(item[2]); break;
                case ">=": auxFilter = el => parseFloat(el[funcGetAtt](item[0])) >= parseFloat(item[2]); break;
                case undefined: auxFilter = el => el[funcGetAtt](item[0]); break;
            }
            if (element === document)
...
PersonalizeTitle.iniciaTitle = function (_style) {
    PersonalizeTitle.titleEl = getElement("#" + PersonalizeTitle.titleName) ||
        getElement(":body")[0].appendChild(document.createElement("div").config({ id: PersonalizeTitle.titleName }));
    ...
    var getPlace = evt => PersonalizeTitle.titleEl.config({ style: { left: (evt.pageX + 12) + "px", top: (evt.pageY + 20) + "px" } })
...
PersonalizeTitle.refreshTitle = function () {
    ...
    getElement(":title").forEach(function (el) {
        ...
        if (parent.tagName !== "HEAD") {
            ...
            el.remove();
            parent.onmouseover = function () { PersonalizeTitle.showTitle(parent.getAttribute(PersonalizeTitle.titleName)) };
...
PersonalizeTitle.showTitle = function (text) {
    text ? show(PersonalizeTitle.titleEl, true, 40) : show(PersonalizeTitle.titleEl, false);
